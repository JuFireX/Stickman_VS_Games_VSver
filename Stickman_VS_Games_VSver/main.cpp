...
int main()
{
    ...
    // 第一关报幕
    directOutput("\n突然!\n", 1);
    streamOutput("再临的屏幕开始闪烁!!!", 10, 1);
    streamOutput("天崩地裂...", 30, 1);
    streamOutput("坠落......", 60, 2);
    directOutput("\n啪唧!!!\n", 1);
    streamOutput("再临掉在了一处深不见底的平台...", 10, 1);
    streamOutput("环顾四周, 再临发现自己来到了推箱子的场地.", 10, 1);
    streamOutput("墙壁非常光滑, 看不出一丝门的痕迹.", 10, 1);
    streamOutput("眼下只能尝试完成这古老的游戏了...", 10, 1);
    directOutput("\n(以完成推箱子为目标继续行动)\n", 0);
    system("cls");
    ...
    do
    {
        choice = choiceOutput("再临决定稍作停留:", { "观察左侧石门", "观察右侧石门", "结束观察" });
        switch (choice)
        {
        case 1:
    ...
    // 第二关报幕
    streamOutput("再临似乎只能前往马里奥平台(?)", 10, 1);
    streamOutput("如果他不打算往坑里跳的话.", 10, 1);
    streamOutput("于是再临向门外走去.", 10, 1);
    streamOutput("是马里奥一样的平台.", 10, 3);
    streamOutput("......", 60, 1);
    streamOutput("再临向前走去,不远处的半空漂浮着\"幸运方块\"...", 10, 1);
    streamOutput("......", 60, 1);
    streamOutput("敲了敲\"幸运方块\",地上掉落了一个遥控器一样的方盒子.", 10, 1);
    system("cls");
    ...
    // 第二关分支
    streamOutput("\n再临成功完成了推箱子，发现遥控器方盒子可以打开!", 10, 1);
    streamOutput("遥控器方盒子里面有一张纸条，上面写着谜题...", 10, 1);
    streamOutput("解开谜题才能打开下一扇门...", 10, 1);
    do
    {
        choice = choiceOutput("再临决定稍作停留:", { "解开谜题", "观察遥控器", "结束观察" });
        switch (choice)
        {
        case 1:
            streamOutput("再临开始解开谜题...", 10, 1);
        ...
        case 2:
            ...
            streamOutput("再临无法从遥控器中得到更多信息...", 10, 1);
            break;
    ...
    // 第三关报幕
    streamOutput("再临发现遥控器方盒子打开了一扇通往神秘世界的门...", 10, 1);
    streamOutput("门后是迷雾缭绕的走廊，尽头有一扇明亮的大门...", 10, 1);
    streamOutput("再临走了进去，发现这是经典的俄罗斯方块游戏...", 10, 1);
    directOutput("\n(以解开最后一扇门为目标继续行动)\n", 0);
    system("cls");
    ...
    streamOutput("再临回到了现实世界，看着手中的遥控器，心中充满了成就感...", 10, 0);
    directOutput("\n(按任意键退出游戏)", 0);
